...
\hspace*{2em}Il peut toutefois être intéressant de recourir exceptionnellement à du texte simple pour réaliser des titres subdivisant plus finement le contenu interne ou externe d’une section :

\textbf{Voici un exemple}
\begin{itemize}
\item C’est un petit titre qui permet donc d’ordonner une section.
Cependant, il est rarement nécessaire : l’organisation en items au sein d’une section suffit le plus souvent.
\item En dessous, j’écris du contenu, ou des items.
\item C’est un cas d’utilisation assez exceptionnel, mais il est à privilégier plutôt que d’utiliser un item suivi de nombreux sous-items !
Cela reviendrait à perdre un niveau de hiérarchie inutilement.
\end{itemize}

\section{Autre section typique}
